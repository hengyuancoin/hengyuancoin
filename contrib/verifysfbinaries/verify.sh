...
###   This script attempts to download the signature file SHA256SUMS.asc from hycoin.org
...
WORKINGDIR="/tmp/hycoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://hycoin.org/bin/"
VERSIONPREFIX="hycoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'hycoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Hycoin Core binary release signing key installed?"
   fi
