Hycoin Core version 0.11.0 is now available from:

  <https://hycoin.org/bin/hycoin-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Hycoin Core or other software:
...
defaults to 0.00001. This will cause transactions with fewer HYC/kB fee to be
...
For example, add the following to `hycoin.conf`:
...
related to the blockchain in the hycoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Hycoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Hycoin Core will 
...
For an example script see [hycoin-submittx](https://github.com/laanwj/hycoin-submittx).
...
- #5334 `cf87536` libhycoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new hycoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only HycoinAmountField
- #5753 `0f44672` Add hycoin logo to about screen
...
- #5809 `46bfbe7` Add hycoin-cli man page
...
- #5849 `41259ca` contrib/init/hycoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Hycoin-Qt.app to Hycoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for hycoin-tx)
